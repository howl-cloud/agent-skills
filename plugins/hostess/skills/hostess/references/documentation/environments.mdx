---
title: Environments
description: Target any environment from the CLI and configure per-environment dials.
---
...
| `services` | object | Per-service overrides for this environment. Each entry sets `replicas` for that service. |
...
If `--env` is omitted, the CLI deploys to the `production` environment by default.
...
Use `environments.<env>.services.<name>.replicas` when one environment should keep a steady instance count and another should sleep. The override replaces the service-level `replicas` for that environment only — the same idea as per-environment domains.
...
After the first deploy with a new override, confirm it took effect: production should hold a steady instance count (here, one), while preview keeps sleeping between visits. See [Configure Autoscaling](/guides/autoscaling) for scale-to-zero patterns.

Replica override rules:

- Keys under `environments.<env>.services` must match services declared in top-level `services:`.
- Only the `replicas` key is allowed on each service override.
- Omitting the override leaves the service-level `replicas` unchanged.
