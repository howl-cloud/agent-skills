...
| Field | Type | Required | Description |
|---|---|---|---|
| `type` | string | **Yes** | Service type (see [Service Types](#service-types)). One of `nextjs`, `fastapi`, `static`, `postgres`, `redis`, or `custom`. |
...
| `visibility` | string | No | Who can reach the service: `public`, `team`, or `private`. See [Visibility](#visibility). |
| `ports` | array | No | Ports to expose. Defaults depend on service type. Optional per-port `visibility`. |
| `resources` | string or object | No | Resource allocation. Defaults to automatic workload-aware sizing (see [Resources](#resources)). |
| `replicas` | number or object | No | Replica count or autoscaling config. Defaults to `1`, or `{ min: 0, max: 1 }` for `static`. |
...
```yaml
type: nextjs | fastapi | static | postgres | redis | custom
```
...
| `static` | File server | `8080` | `GET /` | Vite, Astro, Docusaurus, and HTML directories |
...
**Static** (`static`) uses `source:`. Hostess Native Build produces an HTTP file server from that directory.
...
When you run `hostess deploy`, Hostess uses the ignore rules to select the project files to upload. Hostess builds each service from its `source:` or `build.source` path.

Use **`.gitignore`** for files that Git must ignore. Hostess uses the root `.gitignore` file and all nested `.gitignore` files.

Use a root **`.hostessignore`** file for files that only Hostess must ignore. Put this file next to `hostess.yml`. If `.hostessignore` does not exist, Hostess uses `.dockerignore`.

Specify the build path with **`source:`** or **`build.source`**. Hostess includes this path if `.gitignore` or `.dockerignore` excludes it.

Hostess does not upload `.git`, `.env`, or `.env.*`. Hostess uploads `.env.example`, `.env.sample`, `.env.template`, and names with these suffixes. For example, Hostess uploads `.env.production.example`.

Do not exclude a required source path or Dockerfile in `.hostessignore`. The same restriction applies to `.git`, `.env`, and `.env.*`. If a rule excludes a required file, `hostess deploy` stops before the upload. The error identifies the path and the rule.

Hostess uploads symbolic links that point to files in the project.

A source upload has these limits:

- **500 MiB** compressed size
- **2 GiB** expanded size
- **100,000** files
- **1 GiB** for each file

```bash title=".hostessignore"
# Files that Hostess does not upload
docs
.github
scripts/local
...
- **Waiting** — For **HTTP** services (for example `nextjs` / `fastapi` / `static` / `custom` with an HTTP health check), Hostess waits until the configured HTTP health path responds. For **Postgres** and **Redis**, it waits until the dependency **accepts connections** on the database port.
...
The `visibility` field controls who can open a service’s HTTPS URL:

| Value | HTTPS URL | Access |
|---|---|---|
| `public` | Yes | Anyone |
| `team` | Yes | Organization members after Hostess sign-in |
| `private` | No | Other services via magic variables; local access with `hostess connect` |

Defaults: `nextjs` and `static` → `public`; other types → `private`. Set `visibility: team` explicitly when you want a sign-in–gated URL.
...
Full behavior, Studio Access activity, and custom-domain notes are in [Service Visibility](https://docs.hostess.sh/docs/service-visibility).
...
Some services expose multiple ports. List them all, and optionally set `visibility` per port:
...
Each public or team port gets its own external URL:
...
| Preset | CPU request | CPU limit | Memory | Storage (databases only) | Typical Use Case |
|---|---|---|---|---|---|
| `small` | 0.1 cores | 0.5 cores | 512Mi | 10Gi | Development, small apps |
| `medium` | 0.25 cores | 1.0 cores | 1Gi | 20Gi | Production APIs, standard workloads |
| `large` | 0.5 cores | 2.0 cores | 2Gi | 50Gi | High-traffic services |
| `xlarge` | 1.0 cores | 4.0 cores | 4Gi | 100Gi | Large databases, ML workloads |

Presets are **burstable**: the CPU request is a quarter of the CPU limit, so a service reserves a fraction of a core on the node but may burst up to its limit. Memory request always equals memory limit, because memory cannot be reclaimed by throttling.

If you omit `resources`, Hostess selects automatic workload-aware sizing based on service type. This is not a named preset — use `small`, `medium`, `large`, or `xlarge` when you want a fixed preset.

| Workload | CPU request | Memory request | CPU limit | Memory limit |
|---|---|---|---|---|
| Next.js / FastAPI | 0.1 cores | 256Mi | 1.0 cores | 1Gi |
| Custom | 0.25 cores | 512Mi | 1.0 cores | 1Gi |
| Jobs | 0.1 cores | 256Mi | 1.0 cores | 1Gi |
| Postgres / Redis | 0.25 cores | 1Gi | 1.0 cores | 1Gi |

For Postgres, Redis, and custom services with persistent storage, disk defaults to **20Gi** when storage is omitted.
...
`cpu` sets the CPU **limit**, and the request is derived the same way as presets: a quarter of the limit, with a floor of `0.1` cores. So `cpu: 4.0` above reserves 1 core and may burst to 4. `memory` sets request and limit together.
...
|---|---|---|---|
| `min` | number | - | Minimum number of replicas, always running. Set `0` to let the service idle at zero instances and wake on the next request. |
| `max` | number | - | Maximum number of replicas. Upper scaling bound. |
| `target_cpu` | number | `70` | Target CPU utilization percentage. When average CPU usage exceeds this, Hostess scales up. Applies when `min` is 1 or more. |
...
<Callout type="info">
If you do not specify `replicas`, it defaults to `1` (`static` defaults to `{ min: 0, max: 1 }`). With `min` at 1 or more, scaling follows CPU utilization; with `min: 0` it follows incoming request load — see [Configure Autoscaling](/guides/autoscaling).
</Callout>

#### Validation

- Fixed replica count must be at least **1**.
- Autoscaling `min` can be **0** for eligible `nextjs`, `fastapi`, `static`, and `custom` services — see [Configure Autoscaling](/guides/autoscaling).
- Autoscaling `max` must be at least **1** and greater than or equal to `min`.
...
| `resources` | No | Same shape as service resources. Defaults to automatic workload-aware sizing (see [Resources](#resources)). |
...
```
Port 1: https://my-app-minio-quiet-harbor.hostess.run
Port 2: https://my-app-minio-quiet-harbor-2.hostess.run
```
...
| Field | Default Value |
|---|---|
| `resources` | Automatic workload-aware sizing (see [Resources](#resources)) |
| `replicas` | `1` |
