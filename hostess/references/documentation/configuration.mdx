...
| Postgres / Redis | 1.0 cores | 1Gi | 1.0 cores | 1Gi |
...
- Autoscaling `min` can be **0** for `nextjs`, `fastapi`, and `custom` services that have exactly one `public` or `team` port and no persistent storage; every other service uses **1** or more. `private` ports are unrestricted — only `public` and `team` ports count toward that one.
