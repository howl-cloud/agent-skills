...
On a `type: static` service, install the browser SDK and call it once from the app entry:
...
`inject()` powers the **Analytics** tab; `injectSpeedInsights()` powers **Speed Insights**.
...
- Web Analytics is available for **Next.js** via `@hostess/nextjs` and **static** services via `@hostess/browser`.
