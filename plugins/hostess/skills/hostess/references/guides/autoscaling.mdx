...
`type: static` already defaults to `{ min: 0, max: 1 }` and sleeps when idle. Pin a warm replica in any environment with `environments.<env>.services.<name>.replicas: 1`. See [Static](https://docs.hostess.sh/docs/service-types/static).
