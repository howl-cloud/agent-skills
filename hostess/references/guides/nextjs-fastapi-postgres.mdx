...
✓ Deployment complete (dep_abc123)
  ...
  frontend: https://simple-full-stack-frontend-able-arc.hostess.run
  api:      https://simple-full-stack-api-calm-brook.hostess.run
```
...
```
https://simple-full-stack-frontend-able-arc.hostess.run
```

Check the API:

```bash title="Terminal"
curl https://simple-full-stack-api-calm-brook.hostess.run/api/status
```

Create another database-backed event:

```bash title="Terminal"
curl -X POST https://simple-full-stack-api-calm-brook.hostess.run/api/events \
...
- **[Database Migrations](/guides/database-migrations)** — Deep dive into migration strategies
- **[Custom Domains](/guides/custom-domains)** — Add branded frontend and API domains
- **[Deploy Specific Services](/guides/per-service-deploys)** — Deploy only the frontend or backend independently
- **[Managing Secrets](/guides/managing-secrets)** — Add API keys and environment-specific secrets
