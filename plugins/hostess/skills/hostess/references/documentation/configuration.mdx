...
Defaults: `nextjs` → `public`; other types → `private`. Set `visibility: team` explicitly when you want a sign-in–gated URL.
...
<Callout type="info">
If you do not specify `replicas`, it defaults to `1`. With `min` at 1 or more, scaling follows CPU utilization; with `min: 0` it follows incoming request load — see [Configure Autoscaling](/guides/autoscaling).
</Callout>

#### Validation

- Fixed replica count must be at least **1**.
- Autoscaling `min` can be **0** for eligible `nextjs`, `fastapi`, and `custom` services — see [Configure Autoscaling](/guides/autoscaling).
